...
internal struct SpendingByCategoryChart: View {
    ...
    private var sectionHeader: some View {
        Label("Chi tiêu theo danh mục", systemImage: "chart.pie.fill")
    ...
    private var donutChart: some View {
        ...
        .accessibilityLabel(String(localized: "Biểu đồ chi tiêu theo danh mục"))
        .accessibilityValue(categoryChartSummary)
    }

    private func selectedAnnotation(
        _ item: CategoryBreakdownItem
    ) -> some View {
        VStack(spacing: 2) {
    ...
    private var chartEmptyState: some View {
        VStack(spacing: Spacing.sm) {
            ...
            Text(String(localized: "Chưa có dữ liệu chi tiêu"))
                .font(Typography.bodyMedium)
                .foregroundStyle(Color.onSurfaceVariant)
        }
    ...
    private func sectorOpacity(
        for item: CategoryBreakdownItem
    ) -> Double {
    ...
    private var categoryChartSummary: String {
        let top = data.prefix(3)
